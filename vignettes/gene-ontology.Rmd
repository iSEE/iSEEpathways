...
In this vignette, we use the example of a simulated pathway analysis workflow to demonstrate how one may work with Gene Ontology biological pathways.

In particular, we use the package `r BiocStyle::Biocpkg("org.Hs.eg.db")` to fetch gene sets,
and we define a custom function to display information given the identifier of the pathway currently selected in a panel.
...
In this case, we first create an empty `?SummarizedExperiment-class` object, without any simulated count data nor metadata, as we will not be using any of those data in this example.
...
We also configure configure the app option `PathwaysTable.select.details` to define a function that displays information about the currently selected GO term.

Although not essential, this is a user-friendly and immediate way to translate database identifiers into a human-friendly description.
