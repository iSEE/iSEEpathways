...
Embed the `r BiocStyle::Biocpkg("fgsea")` results in a `r BiocStyle::Biocpkg("SummarizedExperiment")` object.

In this case, we simulate count data, using the same feature identifiers as we used in the pathway analysis.
This will make it to demonstrate how the set of features associated with a pathway can be transmitted and highlighted in another panel in the the user interface.
