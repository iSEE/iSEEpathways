...
But first, we reoder the results by increasing p-value, convert the `?data.table` to a `?data.frame`, and set the `rownames` to the pathway identifier.

```{r}
fgseaRes <- fgseaRes[order(pval), ]
fgseaRes <- as.data.frame(fgseaRes)
rownames(fgseaRes) <- fgseaRes$pathway
metadata(airway) <- list("iSEEpathways" = list("fgsea" = fgseaRes))
```
...
SCREENSHOT("screenshots/integration.png", delay=20)
```
