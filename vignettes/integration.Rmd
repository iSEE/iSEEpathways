...
The function definition must include an ellipsis to accommodate optional arguments passed to other variants of the function.
For instance, see section [Trading off memory usage for speed](#speed-mapping).

```{r}
map_GO <- function(pathway_id, ...) {
    select(org.Hs.eg.db::org.Hs.eg.db, pathway_id, "SYMBOL", keytype = "GOALL")[["SYMBOL"]]
}
...
airway <- embedPathwaysResults(fgseaRes, airway, name = "fgsea (p-value)", class = "fgsea", pathwayType = "GO")
airway
```
...
```{r}
stats <- log2FoldChange(contrastResults(airway, "Limma-Voom")) *
...
fgseaRes <- fgsea(pathways = pathways, 
                  ...
                  maxSize  = 500)
fgseaRes <- fgseaRes[order(pval), ]
airway <- embedPathwaysResults(fgseaRes, airway, name = "fgsea (p-value & fold-change)", class = "fgsea", pathwayType = "GO")
airway
```
...
app <- iSEE(airway, initial = list(
  PathwaysTable(ResultName="fgsea", Selected = "GO:0046323", PanelWidth = 4L),
  VolcanoPlot(RowSelectionSource = "PathwaysTable1", PanelWidth = 4L),
  ComplexHeatmapPlot(RowSelectionSource = "PathwaysTable1",
      PanelWidth = 4L, PanelHeight = 700L,
      CustomRows = FALSE, ColumnData = "dex",
